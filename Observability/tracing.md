...
除了方法增强的方案外，还有**基于边车代理的追踪**。这是服务网格的专属方案，也是最理想的分布式追踪模型。**边车代理对应用完全透明**：有自己独立数据通道，追踪数据通过控制平面上报，不会有任何依赖和干扰。**与程序语言无法**：无论应用采用什么编程语言，只要它通过网络（HTTP或 gRPC）访问服务，就可以被追踪到。

现在，市场占有率最高的 Envoy 就提供了完善的追踪功能，SkyWalking、Zipkin、Jaeger 等系统也可以兼容来自 Envoy 的追踪数据。如果非要总结种方式的实现有什么缺点，就是边车代理本身对应用透明的原理，决定了它只能实现服务调用层面的追踪。
