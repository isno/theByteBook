...
如图 9-10，展示了一套基于 Elastic Stack 完整的处理日志方案。这套方案中，Beats 组件部署到日志所在地收集原始数据，然后使用 MQ 做缓冲换取更好的吞吐，接着发给 logstash 组件做数据清洗，最后落地到 Elasticsearch 集群并进行索引，使用时通过 Kibana 组件进行可视化、检索和分析，如果有必要挂上 Nginx 做各类访问控制。
