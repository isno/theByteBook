...
日志的处理和分析成了典型的大数据场景之一：高吞吐写入、海量存储、还要求实时文本检索。实现完整的日志方案也成了整个技术架构中难度最大，技术深度要求最高的部分。

这一节，笔者从底层原理到成本分析介绍业内常用的 Elastic、ClickHouse、Loki 三种方案。
...
讨论实现一套完整的日志系统，工程师们或多或少都应该听说过这几个名词：ELK、ELKB 或者 Elastic Stack。这些其实是实现日志处理方案一套开源组件，组件之间作用不用，可以像网络协议一样按分层来理解和归类，如下图所示，形成一个类似 TCP/IP Stack 类似的 ELK Stack（这里的 Stack 肯定不是协议栈，且和协议没有任何关系）。为统一明确，本文把上面的所有名词简称为 Elastic。
...
Elasticsearch 在日志场景中的优势在于全文检索能力，快速从海量的数据中检索出关键词匹配的日志，其底层核心技术就是 Lucene 中的倒排索引（Inverted index）。

:::tip 倒排索引

倒排索引（inverted index）也常被称为反向索引，是信息检索领域常用的索引技术，将文本分割成一个个词，构建 词 -> 文档编号 的索引，可以快速查找一个词在哪些文档出现。
