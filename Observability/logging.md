...
此外，列式存储和数据压缩通常是伴生的。数据压缩的本质是通过一定的步长对数据进行匹配扫描，发现重复部分后进行编码转换。因此，数据中重复项越多，压缩率越高。面向列式存储，同一列字段的数据具有相同的数据类型和语义，重复项的可能性自然更高。由于，每一列的数据类型不同，为每一列设置不同的压缩算法，数据的压缩率显然更高。ClickHouse 支持为不同的列配置不同的压缩算法，如下所示。我们对 DateTime 列使用了 Double-Delta，对 Float 列使用了 Gorilla，对 String 列使用了 LZ4。

```shell
CREATE TABLE example (
    id UInt64 CODEC(ZSTD), -- 使用 ZSTD 压缩
    name String CODEC(LZ4), -- 使用 LZ4 压缩
    age UInt8 CODEC(NONE),  -- 不压缩
    score Float32 CODEC(Gorilla) -- 使用 Gorilla 压缩
    createTime DateTime CODEC(Double-Delta), -- 使用 Double-Delta 压缩
) ENGINE = MergeTree()
ORDER BY id;
```
