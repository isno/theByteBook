...
3. 第四部分，解释近几年基础设施的变革，我们先了解 Google 内部系统是怎么演变的，再来学习 Kubernetes 中关于计算、网络、存储逻辑中设计（第 7 章）。然后，根据服务间通信的演化，搞清楚服务网格技术是如何出现的、解决了什么问题（第 8 章）。接下来的课题是如何通过系统运行状态的蛛丝马迹，保障复杂分布式系统的可靠运行（第 9 章）。最后第 10 章是提高生产力的秘籍，将介绍一种以声明式系统为基座、以 Git 为单一可信源的交付模型。
