...
本书主要是研究系统整体如何运行，内容源于笔者对软件架构认知/实践的总结。架构原理确实是一个足够庞大的话题，介绍清楚会延伸出无数脉络。

### 第一部分：绪论

本书参考当前技术发展趋势，并结合落地实践总共分为十章，整体内容脉络从架构外部（用户端网络）逐渐延伸至架构内部（内核网络、虚拟网络、负载均衡、分布式、质量观测、GitOps 交付等），以下为各章内容简介：

- 第一章，我们先从需求的背景、发展的历史开始，漫谈微服务、容器、不可变基础设施、声明式设计、服务网格、DevOps 等云原生技术。从解决问题的角度理解这些技术，借历史之名审视今天的现实问题，以及思考未来的技术/架构演进之路。
- 第二章，我们从一道经典的面试题“浏览器打开 url 到页面展现，中间发生了什么？”开始，逐步探究 DNS、HTTP、SSL、QUIC 和传输层等各个环节原理以及该如何一步步构建“足够快”的网络服务。
- 第三章：我们开始修炼内功，深入 Linux 内核网络，了解 Linux 内核、netfilter、虚拟网络技术，为后续 Kubernetes、容器、服务网格等篇节储备一些基础知识。
...
- 第九章：我们从遥测数据认识可观测性，对比可观测性以及传统监控的区别。再探讨事件日志、链路追踪和聚合度量三个领域中等各个方案落地权衡。
- 第十章：
