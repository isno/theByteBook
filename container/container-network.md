...
最后再介绍一种 underlay 模式，underlay 就是 2 层互通的底层网络，传统网络大多数属于这种类型。

这种模式，一般使用 MACVLAN 技术，配置的容器网络同主机网络在同一个 LAN 里面，可以具有和主机一样的网络能力，并且没有 Linux Bridge 等方式带来的 Bridge 处理和地址翻译的负担。因此，underlay 模式能最大限度的利用硬件的能力，往往有着**最优先的性能表现**，但也由于它直接依赖硬件和底层网络环境限制，必须根据软硬件情况部署，没有 overlay 那样开箱即用的灵活性。
