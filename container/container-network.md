...
Linux 内核 3.12 版本起已经对 VXLAN 技术支持完备，经过简单的配置，就能在宿主机网络之上，构建一个虚拟的通信通道，也就是隧道。容器间之间的通信，通过隧道的两个端点设备（VETP）对数据封包/解包。这样，通信的两方（容器）认为它们是在二层网络通信，但实际上靠的是宿主机三层网络。

Flannel 基于 VXLAN 技术的覆盖网络通信逻辑，总结为图。
