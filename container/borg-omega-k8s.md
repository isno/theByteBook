...
2013 年夏天，Google 的工程师们开始讨论借鉴 Borg 的经验进行容器编排系统的开发，并希望借此在错失的云计算市场中施加 Google 的技术影响力。Kubernetes 项目获批后，Google 在 2014 年 6 月的 DockerCon 大会上正式宣布将其开源。
...
#### 1.1 发展历史：从 chroot 到 cgroups

- 历史上，最初的容器只是提供了 root file system 的隔离能力 （通过 chroot）；
- 后来 FreeBSD jails 将这个理念扩展到了对其他 namespaces（例如 PID）的隔离；
- Solaris 随后又做了一些前沿和拓展性的工作；
- 最后，Linux control groups (cgroups) 吸收了这些理念，成为集大成者。 内核 cgroups 子系统今天仍然处于活跃开发中。
...
容器提供的资源管理工具使以上需求成为可能，再加上强大的内核资源隔离技术，就能避免这两种类型任务的互相干扰。但这种隔离并未达到完美的程度，容器无法避免那些不受内核管理的资源的干扰，例如三级缓存（L3 cache）、 内存带宽；此外，还需要对容器加一个安全层（例如虚拟机）才能避免公有云上各种各样的恶意攻击。
...
容器提供的资源隔离和依赖最小化特性，仍然使得它在 Google 内部非常成功， 因此容器成为了 Google 基础设施唯一支持的可运行实体。这带来的一个后果就是， Google 内部只有很少几个版本的操作系统，也只需要很少的人来维护这些版本，以及维护和升级服务器。
