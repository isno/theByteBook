...
随着容器化和微服务架构大规模应用，手动管理容器实例、微服务的方式变得异常艰难。

为了减轻手动管理容器的心智负担，实现容器调度、扩展、故障恢复等自动化管理，容器编排系统应运而生。过去十年间，Kubernetes 发展成为容器编排系统的事实标准，成为大数据分析、机器学习以及在线服务等领域广泛认可的最佳技术底座。
