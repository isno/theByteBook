...
本节，笔者将从最初的文件系统隔离开始，逐步介绍容器在不同历史阶段的作用，深入理解容器技术的演进及 Kubernetes 中最核心的概念 Pod 的设计背景。
...
如果这两个 Pod 需要协作并运行在同一台机器上，调度器可能首先将 Nginx 调度到 Node1。但由于 Node1 上只剩下 1.25G 的内存，而 Nginx 占用 1G 内存，LogCollector 将无法在 Node1 上运行，因为资源不足，从而导致调度流程被阻塞。尽管可以通过重新调度来解决这个问题，但考虑到如果需要解决数以万计的容器协同调度问题呢？以下为业内两种典型的解决方案：

- 成组调度：可以在集群中等待足够的空余资源来满足亲和性约束容器的需求后，统一进行调度。这是一种典型的解决方案，但可能导致调度效率降低、资源利用不充分，并且可能出现互相等待而产生死锁的问题；
- 提高单任务调度效率：通过提高单任务调度的效率来解决问题。例如，Google 的 Omega 系统介绍了一种基于共享状态并通过乐观锁解决并发导致的资源冲突的方法，但这种方案非常复杂。


将运行资源的需求声明直接定义在 Pod 上，并以 Pod 作为最小的原子单位来实现调度，Pod 与 Pod 之间不存在超亲密的关系。如果有关系，就通过网络通信实现关联。复杂的协同调度问题在 Kubernetes 中直接消失了。
