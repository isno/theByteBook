...
Sidecar 模式其实就是在 Pod 里面定义一些专门的容器，通过职责分离与容器的隔离特性，能够降低容器的复杂度，同时能扩展并增强已有容器的功能。
...
如上图所示，这些容器通过提供额外的服务或功能（如日志记录、监控、安全性或数据同步）来增强或扩展主应用容器的功能，让专业的人去做专业的事成为可能，而且使开发一个高内聚、低耦合的软件变的更加容易。
