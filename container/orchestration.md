...
从字面上，“Container” 这个词，确实很难让人形象地理解其真正含义，容器编排生态中的另外一个词 “Pod” 也是如此。仅看一些概念性的解释并不能给我们清晰的认识，甚至会引起误解，比如把容器比作轻量化虚拟机(VM)。

:::tip 如果容器类似虚拟机
那就应该有一种普适的方法把虚拟机里面的应用无缝地迁移到容器中，可是现实世界中并没有这种方法。
:::

为了尽可能深入理解容器，我们从一个最原始的文件系统隔离讲起。
...
上面的代码首先创建一个新的根目录（new-root），然后将 Bash shell 以及依赖项复制到 new-root，并运行 chroot 创造一个 jail 。
...
/bin
```
看起来跟 chroot 的操作差不多，也创造了一个隔离的文件系统环境，那这是否意味着 chroot 就是容器了呢？ 绝对不是，**chroot 只是改变了根目录，而非创建了真正的独立隔离、安全的环境**：chroot 后的进程、文件系统、设备等等都没有被隔离。

### 文件系统没有被完全隔离

调用 chroot 后，使用相对路径仍然可以引用新根目录之外的文件，对于 chroot 的进一步调用也会覆盖当前的 jail。如下代码，在 jail 之内改变相对路径再创造一个新 jail，这样就从 jail 逃逸了。
...
15380
```
可以看到运行的 /bin/bash 并非是1号进程。chroot 之后的进程并没有和宿主机隔离。我们还可以通过 mount 挂载外部的 /proc ，看到外部进程的情况，甚至杀死监狱之外的进程。
...
Linux namespaces 是由内核直接提供的全局资源封装，针对进程设计的访问隔离机制，使得名称空间内的进程看起来拥有自己的全局资源隔离实例。

早期提出以及使用要追溯到贝尔实验室开发的一个名叫 Plan 9 的分布式操作系统（就是上面提到的设计 chroot 的 unix 系统）。最初的目的是为了实现文件的隔离，并非为了容器而设计。后来 Linux 吸收了这些理念，从 2.4.19 引入了 Mount 名称空间，陆续添加了 UTS、IPC、PID、Network、User 等名称空间，成为集大成者。
...
<div  align="center">
  <img src="../assets/sidecar.png" width = "500"  align=center />
  <p>边车模式的形象示例</p>
</div>
