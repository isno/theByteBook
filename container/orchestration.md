...
如今当我们谈起容器，多数内容会将容器解释为“软件标准单元”，以及告诉我们“为什么要使用容器”。这些内容只是一些概念性的解释，没有对底层技术有过多解密，这就导致人们常常将容器跟虚拟机（VM）对比，从而逐渐远离技术真相。

:::tip 如果容器类似虚拟机
那就应该有一种普适的方法把虚拟机里面的应用无缝地迁移到容器中，可是现实世界中并没有这种方法。
:::

我们思考一下：“容器到底是什么？”。从字面上，“Container” 这个词，确实很难让人形象地理解其真正含义，容器编排生态中的另外一个词 “Pod” 也是如此。仅看一些概念性的解释并不能给我们清晰的认识。为了尽可能深入理解容器，我们沿用惯例，从历史的起源讲起。

## 文件系统隔离

1979 年，Unix 系统引入了一个革命性的技术 -- chroot（chang root）。这个命令允许管理员将进程的根目录锁定在指定的位置，从而可以有效的限制该进程访问的文件系统范围，实现基本的文件系统隔离。
...
/bin
```
看起来跟 chroot 的操作差不多，那 chroot 是不是就算是容器了呢？

绝对不是，**chroot 只是改变了根目录，而非创建了真正的独立隔离、安全的环境**：

### 文件系统没有被完全隔离

系统调用 chroot 时，在当前工作目录下，使用相对路径仍然可以引用新根目录之外的文件。另外对于 chroot 的进一步调用会覆盖当前的监狱。

如下代码，通过覆盖当前的监狱创建一个新的监狱，并将直接改为 chroot 之外的某个相对路径，这样就从监狱逃逸了。
```
#include <sys/stat.h>
#include <unistd.h>
int main(void) {  
...
```
仅仅通过 chroot 运行的 /bin/bash 并非1号进程，我们可以通过 echo $$ 打印当前 shell 的 PID。


:::tip pivot_root

如今，容器运行时不再使用 chroot，而是使用取代 chroot 的 pivot_root。
:::
...
我们还可以通过 mount 挂载外部的 /proc ，可以看到外部进程的情况，甚至杀死监狱之外的进程。
...
Linux namespaces 是由内核直接提供的全局资源封装，针对进程设计的访问隔离机制，使得名称空间内的进程看起来拥有自己的全局资源隔离实例。早期提出以及使用要追溯到贝尔实验室开发的一个名叫 Plan 9 的分布式操作系统。最初的目的是为了实现文件的隔离（即 chroot），并非为了容器而设计。
