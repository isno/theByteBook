...
所谓的“容器镜像”，其实就是一个特殊的压缩包。容器系统创建一个“沙盒”，在“沙盒”内解压镜像，然后就可以运行你的应用程序了。
...
$ docker run 镜像名称
```
上述“沙盒”，其实就是用 Namespace 和 Cgroups 技术创建出来的隔离环境。

由于镜像打包的是“整个操作系统”，应用以及它运行所需的依赖全部被封装在了一起，从而赋予了容器最核心的一致性能力：无论本地还是云端某个虚拟机，只要解压打包好的容器镜像，应用运行所依赖的环境就能重现。
