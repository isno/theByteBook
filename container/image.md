...
Dragonfly 就是基于 P2P 网络实现的容器镜像分发加速系统。Dragonfly 提供了一种无侵入（不用修改容器、镜像仓库）的镜像下载加速解决方案，它的工作流程如图 7-10 所示。
...
最后总结，先是编译阶段生成足够小的镜像，然后使用 P2P 的方式加速镜像下载，最后再利用 Nydus 项目的镜像层延迟加载，便具有了瞬时启动几百、几千 Pod 的能力，对于大规模集群或者对冷启动扩容延迟要求较高的场景（大促扩容、游戏服务器扩容等）来说，不仅能大幅降低容器启动时间，还能大量节省网络/存储开销。
