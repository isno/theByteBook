...
容器镜像的大小直接影响容器的启动时间，一些大型软件的镜像可能超过数 GB。如机器学习框架 TensorFlow 的镜像大小为 1.83 GB，该镜像冷启动时至少需要 3 分钟。大型镜像不仅启动缓慢，而且镜像内的文件往往未被充分利用（业内研究表明，通常镜像中只有 6% 的内容被实际使用）。

为实现按需加载镜像文件，降低容器启动时间，蚂蚁、阿里云和字节跳动联合开发了容器镜像加速项目 Nydus。

Nydus 优化了镜像层（layer）的设计结构提升容器启动效率。它将镜像层的数据（blobs）与元数据（bootstrap）分离。容器第一次启动时，Nydus 首先拉取元数据，再按需拉取 blobs 数据，相较于拉取整个镜像层，下载的数据量大大减少。值得一提的是，Nydus 还通过 FUSE 技术（Filesystem in Userspace，用户态文件系统）重构文件系统，用户几乎无需任何特殊配置（感知不到 Nydus 的存在），即可按需从远程镜像中心拉取数据，加速容器的启动。
...
总结上述镜像的应用：在最初的编译阶段生成足够小的镜像。接着，利用分布式节点和智能调度（也就是 P2P 网络）加速镜像下载，减少网络带宽消耗。最后，利用 Nydus 技术实现镜像层的按需加载。最关键的是，上述所有的技术调整，并没有破坏原有应用层流程，对于业务工程师而言，完全是透明的。
