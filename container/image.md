...
事实上，大部分开发者理解的应用依赖一直局限编程语言层面，例如 JAVA 依赖 JDK。但实际上，一直一来一个容易被忽视的事实是，操作系统本身才是应用运行所需的最完整的依赖库。由于 rootfs 打包的并不只是应用，而是整个操作系统的文件和目录，这也就意味着，应用以及它运行所需的所有依赖全部被封装在了一起。
...
比如，笔者用 centos 制作了一个 rootfs，并安装了 java 的运行环境，用来部署 java 应用。那么，笔者的同事想发布一个 Java 应用时，显然想复用之前安装过 Java 运行环境的 rootfs，而不是重新制作一个。其次，如果我们都重新制作 roofs，考虑一台主机通常运行几十个容器，那得占用多大的存储空间呢。
...
下面用代码演示 OverlayFS 的工作原理，代码最后一行使用 mount 命令挂载，其中 -t 参数指定挂载后的文件系统类型为 overlay（也就是 OverlayFS），后面的 lowerdir（OverlayFS 中的只读层，可以指定多个目录）、upperdir（OverlayFS 中的读写层）、merged（挂载后最终呈现给用户的文件系统视图）等为 overlay 的文件系统的参数。
...
挂载后的文件系统视图，如图 7-7 所示。merged 目录本来是一个空目录，使用 OverlayFS 挂载后，用户在 merged 目录内看到完整的各个文件。
...
最终，这 6 个层被联合挂载到 /var/lib/docker/overlay/mnt 目录中，表现为一个带有 JAVA 运行环境的 debian 操作系统供容器内进程使用。
