...
容器镜像的大小直接影响容器启动时间，一些大型软件的镜像可能超过数 GB。例如，用于机器学习的 TensorFlow 镜像大小为 1.83 GB，冷启动该镜像至少需要 3 分钟。

大型镜像不仅启动缓慢，而且镜像内的文件往往并未被充分利用（业内研究表明，一般镜像只有 6% 的内容被实际使用）。如果能够实现按需加载镜像内的文件，肯定能大幅降低，这便是 Nydus 项目出现的背景。
