...
加速镜像下载的有效方式之一是使用 P2P 网络加速技术。Dragonfly 就是基于 P2P 网络实现的容器镜像分发加速系统，它提供了一种无需修改容器或镜像仓库配置的无侵入加速方案。

图 7-9 展示了 Dragonfly 的工作流程。首先，Dragonfly 在多个节点上启动 Peer 服务（类似 P2P 节点）。当容器系统下载镜像时，下载请求通过 Peer 转发到 Scheduler（类似 P2P 调度器），Scheduler 判断该镜像是否为首次下载：
...
可以看出，Dragonfly 的镜像下载加速流程与 P2P 下载加速非常相似，二者的原理都是通过分布式节点和智能调度来加速大文件的传输与重组。
