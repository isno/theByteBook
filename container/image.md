...
不过 Docker 镜像并不是野蛮地把所有的依赖文件打包成某种格式，而是做了一个巧妙的创新：**Docker 基于 UnionFS（Union File System，联合文件系统）采用堆叠的方式，对 rootfs 进行分层设计**。
...
容器镜像的大小会影响容器启动的时间，譬如 tensorflow 的镜像有 1.83 GB，冷启动这个镜像至少需要 3 分钟的时间。这么大的镜像一个问题是启动慢，另外还一个问题是容器镜像中的文件也并不会被充分利用，《Making containers lazy with Docker and CernVM-FS》[^3]的论文中就提到一般镜像只有 6% 的内容会被实际用到。
