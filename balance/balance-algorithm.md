...
考虑服务器的处理能力不同，上面的负载均衡策略又有了对服务器“**加权**”的优化补充（不包括一致性哈希均衡）。加权调度算法增加了按权值高低分配的因素，权值高的服务器优先收到连接，会比权值低的服务器处理更多的连接，最大化保证各个性能不一致的服务器集群整体处于均衡状态。

加权负载均衡策略包括加权轮询调度（Weight Round Robin）、加权最小连接调度（Weight Least-Connection）、加权随机策略（Weight Random）等。