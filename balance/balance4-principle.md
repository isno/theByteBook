...
四层负载的典型软件实现是 LVS（Linux Virtual Server，Linux 虚拟服务器）。通过虚拟化的方式将多台物理服务器（也称为 RealServer 真实服务器）组合成一个对外表现为单一虚拟服务器

现在 LVS 已经是 Linux 标准内核的一部分，在内核空间处理负载均衡和转发逻辑，因此具有很高的处理性能，可支持上百万个并发连接请求。
