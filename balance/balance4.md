...
从 Linux 内核 2.4 版本起，LVS 被集成进 Linux 内核，它在 Netfilter 钩子之上注册回调函数，捕获数据包后根据负载均衡算法转发至“后端服务器”。区别数据包的处理方式，LVS 有了三种工作模式，直接路由（DR，Direct Routing）、隧道（Tunnel）和网络地址转换（NAT，Network Address Translation）。
...
为了避免后端服务器抢答 VIP 的 ARP 请求，导致网络问题，还必须禁用后端服务器的 VIP ARP 应答。命令如下所示：
...
经过上述配置后，后端服务器 lo 接口检测到目标 IP 为 VIP（配置的 1.1.1.1）的数据包时，将其识别为自己的地址并处理请求。处理完成后，后端服务器通过其外部接口（如 eth0）发送响应数据时，会保持原始请求中的目的 IP 地址，即 VIP（1.1.1.1），作为响应数据包的源 IP。


在直接路由模式下，请求通过负载均衡器转发至后端服务器，而后端服务器的响应无需再经过负载均衡器，直接返回给客户端。这种模式形成了请求、转发和响应之间的“三角关系”，因此也被形象地称为“三角传输模式”，如图 4-9 所示。
...
直接路由模式的主要优势在于，特别适合响应流量远大于请求流量的场景。例如，在典型的 HTTP 请求/响应模式中，假设请求流量仅占总流量的 10%，而响应流量占 90%。通过三角传输模式，负载均衡器仅需处理 1/10 的总流量。这种设计不仅大幅降低了带宽成本，还显著提高了负载均衡器的可靠性（流量越少，负载越轻）。

当然，直接路由模式也存在明显的缺陷：
- 监控限制：由于响应流量直接返回客户端，负载均衡器无法监控完整的 TCP 连接状态，从而影响防火墙策略的实施（例如，负载均衡器只能捕获 TCP 连接的 SYN 包，而无法跟踪后续的 ACK 包）。
- 网络架构要求：负载均衡器与后端服务器之间通过链路层通信，要求两者位于同一子网内，这对网络拓扑的设计提出了较高的约束。
...
负载均衡器收到到数据包后，根据调度算法选择出一台后端服务器（172.12.1.3），然后对数据包执行封装处理。
...
将一个 IP 数据包封装在另一个 IP 数据包内，并配合相应的解包机制，这是典型的 IP 隧道技术。Linux 中的 IPIP 隧道实现了字面意义上的“IP in IP”。由于隧道模式工作在网络层，绕过了直接路由模式的网络限制，因此 LVS 隧道模式能够跨越子网进行通信。

在隧道模式下，由于源数据包的所有信息未被修改，保留了三角传输模式的特征（也要继续处理 lo 接口）。隧道模式的整个请求到响应的过程，如图 4-10 所示。
...
隧道模式相当于直接路由模式的升级（支持了跨网）：
- 由于需要对数据包封装/解封，后端服务器必须支持某些隧道技术（如 IPIP、GRE）；
- 其次，**只要是三角模式（LVS 的直接路由模式或者隧道模式）必须保证后端服务器的 lo 接口与负载均衡服务器有相同的虚拟 IP 地址**。这是因为，在回复客户端的数据包时，必须使用 VIP 作为源地址，客户端才能够正确处理返回的数据包。
...
图 4-12 展示的是最常见的主备模式，其核心是每台节点上运行 Keepalived 软件。该软件实现了 VRRP（Virtual Router Redundancy Protocol）协议，虚拟出一个对外服务的 IP 地址（VIP）。该 VIP 默认绑定在主节点（Master）上，由主节点处理所有流量请求。备用节点（Backup）则持续监控主节点的状态，并在主节点发生故障时迅速接管 VIP，从而确保服务不中断。
