...
在那段时期，虽然所有的共识系统都是从 Paxos 开始的，但工程师在实现过程中发现有很多难以逾越的难题，往往不得已又开发出一种与 Paxos 完全不一样的架构，这就导致 Lamport 的证明并没有太大价值。笔者这里继续借用 Chubby 作者的一句话。
...
作为一个强 leader 模型算法，本节笔者就以其中领导者选举、日志复制为线索讲解困扰 Paxos 难以落地的问题，在 Raft 算法中是如何解决的。
