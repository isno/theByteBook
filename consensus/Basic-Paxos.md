...
希望你没有对前篇 Paxos 的「复杂」做的铺垫所吓倒，共识其实已经算是一个古老的领域，30余年间已经有无数简洁直白的视频、论文等资料进行过解读。譬如在网络中流传甚广的 raft 和 paxos 视频讲解[^1]，即使没有多少技术背景，也能通俗地理解 Paxos。这一节，我们从故事回到算法本身，正式开始学习 Paxos。
...
为了克服单点故障问题，借鉴多数派的机制，思路是写入一半以上的节点，如果集群中有 N 个节点，客户端需要写入 W >= N/2 + 1 个节点。使用多数派的机制后最多可容忍 (N-1)/2 个节点故障。但是问题还是存在：**每个决策节点该接受几个提案呢**? 
...
那这样的一个二阶段提交，我们假设下面几种情况看看能不能解决前面的问题。
