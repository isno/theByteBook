...
在学习 VXLAN 之前，我们有必要充分了解一些基础的物理通信基础。接下来，笔者将先介绍 VXLAN 的前身 VLAN（Virtual Local Area Network，虚拟局域网）。
...
为了实现上述中的单臂路由模式，802.1Q 规范中专门定义了子接口（Sub-Interface）的概念，它的作用是允许在同一张物理网卡上，针对不同的 VLAN 绑定不同的 IP 地址。
