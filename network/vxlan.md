...
从 Linux 内核 3.12 版本起，Linux 对 VXLAN 有了完备的支持。在 Linux 系统中，不需要专门的硬件只要三层可达的网络就可以部署 VXLAN 网络，因此具有很高的灵活性、扩展性和可管理性。

目前，VXLAN 已经成为当前构建数据中心的主流技术，绝大多数的公有云的 VPC 都是用 VXLAN 来作为数据转发层面。