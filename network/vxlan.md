...
SDN 网络模型如图 3-16 所示：
- 位于下层的网络称 Underlay 网络，它是由路由器、交换机等硬件设备互联而成的物理网络。
- 位于上层的网络称 Overlay 网络，它是采用网络虚拟化技术在 Underlay 网络之上创建的虚拟网络。
...
- 另一方面，VXLAN 本质是构建了一条穿越不同物理网络的“隧道”，它通过封装原始的 Layer 2（以太网）帧在 Layer 3 网络中传输，使得不同物理网络之间的通信，就像在一个广播域内一样。虚拟机/容器无论是迁移到 VLAN B 还是 VLAN C，仍然处于同一个二层网络，网络层的配置无需对应调整。

VXLAN 具有很高的灵活性、扩展性和可管理性，已经成为构建数据中心与容器网络的主流技术，绝大多数公有云的 VPC（Virtual Private Cloud，虚拟私有云）、容器网络都是利用 VXLAN 技术在一个大二层网络上构建。
