...
自定义链可以看作是对调用它的链的扩展，例如自定义链结束的时候，可以返回内置链，也可以再继续跳转到其他自定义链。自定义链的设计使 iptables 不仅仅只是一个 IP 包过滤工具，它在容器网络中也扮演了重要的角色。例如 Kubernetes 的核心组件 kube-proxy，就是利用自定义链实现了 Service 功能。
...
所以，当 Kubernetes 集群的规模较大时，应该避免使用 iptables 模式。如果容器间通信解决方案使用的是 Cilium，还可以创建没有 kube-proxy 组件的 Kubernetes 集群，利用“内核旁路”技术越过 iptables 影响，全方位提升容器网络性能。 
