...
两者之间的关系如如图 3-6 所示，iptables 在用户空间管理数据包处理规则，内核中 Netfilter 根据 iptables 的配置对数据包进行处理。
...
如图 3-7 所示，一个目的是本机的数据包依次经过 PREROUTING 链上面的 mangle、nat 表，然后再依次经过 INPUT 链的 mangle、filter、nat 表，最后到达本机某个具体应用。

<div  align="center">
	<img src="../assets/iptables-chain.png" width = "450"  align=center />
	<p>图3-7 iptables 中链和表的关系</p>
</div>
...
kubernetes 中 kube-proxy 组件的 iptables 模式就是利用自定义链模块化地实现了 Service 功能，其架构如图 3-9 所示。
...
如图 3-10 所示的基准测试，当 1000 个服务（10000 个 Pod）以上时，开始观察到各个模式的差异。
