...
- 建立连接时握手延迟大：HTTPS 基于 TCP 协议，加上 TLS 握手初次连接至少需要 3 个 RTT 才能建立。
- 队头阻塞问题：以 HTTP/2 多路复用为例，多个数据请求在同一个 TCP 连接上作为不同的流发送，应用层的所有流都必须按顺序处理。如果一个流的数据丢失，后面的其他流的数据将被阻塞，直到丢失的数据被重传。
- TCP 僵化问题：作为一个运行了接近 40 多年的协议，许多中间件（例如防火墙和路由器）已经变得依赖于某些隐式规则，推动TCP协议更新非常苦难。


以上的问题，就是 QUIC 出现的背景。QUIC (Quick UDP Internet Connections) 是一种基于 UDP 封装的安全、可靠传输协议，他的目标是取代 TCP 并自包含 TLS 成为标准的安全传输协议。
...
QUIC 采用 UDP 作为其传输协议，与 TCP 相比具有更低的延迟和更高的吞吐量，并且它还使 QUIC 能够绕过可能干扰 TCP 的网络中间件。 QUIC 包含基于 TLS 1.3 的内置加密协议，可在端点之间提供安全通信。这就是 QUIC 的用武之地 —— 结合了 UDP 的速度和效率、TLS 的安全性，以及 HTTP3，为现代互联网创建可靠和高性能的传输协议。
