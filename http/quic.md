...
如图 2-29 所示，若一个属于 Stream2 的 TCP 数据包丢失（如图中标记为 5 的圆圈所示），将会导致后续数据包的传输被阻塞，这就是我们所称的“队头阻塞问题”（head-of-line blocking）。
...
从请求耗时表现来看（见图 2-30），相同的网络质量下，HTTP/3 的耗时比 HTTP/2 降低了近一半。
:::center
  ![](../assets/quic-1.png)<br/>
 图 2-30 不同网络质量下，各协议耗时表现（耗时单位 ms）
:::

不过，测试中也发现了一个值得注意的问题：根据图 2-31 的网络请求成功率来看，HTTP/3 的失败率明显高于 HTTP/2。
...
- 客户端层面：面临适配与收益之间的成本权衡。采用 QUIC 协议的客户端需要**具备降级容错能力**，并准备在**较长时间内同时维护新旧两种网络库**。
