...
- 域名解析优化：减少域名解析产生的延迟。例如使用预解析，那么 HTTPS 链接就能减少一个 RTT。
- 包体积优化：传输数据的大小与传输耗时成正比。减小传输内容的体积是降低请求耗时的最有效手段之一。
- SSL 层优化：升级 TLS 算法和 HTTPS 证书，例如升级到 TLS 1.3 协议，可以将 SSL 握手的 RTT 从 2 个减少到 1 个。
- 传输层优化：升级拥塞控制算法提高网络吞吐，例如将默认的 Cubic 升级为 BBR。这在弱网环境（大带宽、长链路）下非常有效。
