...
笔者曾负责过弱网环境下提升视频播放流畅度的项目，其中一项改善明显的措施是尝试切换了 TCP 拥塞控制算法，把 Linux 默认的 Cubic 算法替换为 BBR 算法，调整之后网络吞吐量大约提升了 45% ，用户播放体验改善明显，这一节我们就来聊一聊网络传输中最重要的理论 -- 拥塞控制。

网络拥塞控制是一个十分发散的话题，而实践是我们的最终目的，内容就暂且抛开理论，将实践操作和效果作为开篇。
...
这些机制完美适应了 1980 年代的网络特征：低带宽、浅缓存队列。美好持续到了 2000 年代，随后互联网大爆发，多媒体应用特别是图片、音视频类促使带宽猛增，而摩尔定律促使存储设施趋于廉价、网关路由设备队列缓存猛增。链路变长变宽、设备性能提升，初代的拥塞控已经不适用 -- 这便是 BBR 诞生的背景。
