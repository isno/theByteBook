...
在介绍容器技术之前，我们先聊聊需求的本源是什么。在我看来，本源是摩尔定律的发展让资源过剩无法充分利用。

摩尔定律让我们有了越来越多的计算资源可以使用，现在一台普通的PC服务器拥有了非常强大的、能和十年前小型机媲美的计算能力。在容器之前，物理机约定成俗的用法总是切割为1-8vCPU的虚拟机使用。


- 待解决问题本身内在的并行度有限：随着多核多处理器系统的日益普及，IT行业从2004年开始进行串行编程到并行编程的升级改造。但受阿姆达尔定律制约，解决特定问题的并行度超过一定临界点之后收益将逐渐变小。所以一味提高系统并行度并不是经济的做法。
