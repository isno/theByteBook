...
最后总结，对比可变基础设施，**不可变基础设施的最大的优势是一致性**，不同环境的标准化配置成为可能，所有的配置都通过标准化描述文件（例如 yaml、dockerfile 等）进行统一定义，不同的服务对象（例如 Kubernetes 中的 Pod、Service）都按照同样的定义创建，不同实例配置不一致的情况不会再出现。
