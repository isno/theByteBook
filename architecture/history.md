...
虽然容器技术早已出现，但 Docker 创新性地提出了镜像的概念，实现了一种新型的应用打包、分发和运行机制。云原生早期阶段，将 Docker 作为容器技术代表实至名归。
...
图 1-5 形象地概述了这二十年云计算的演进过程：从传统预制 IT、托管到云，以及云服务模型不同的形态如 IaaS、PaaS、CaaS、SaaS 等。
