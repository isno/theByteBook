...
数据平面由轻量级的网络代理（如 Envoy 或 Linkerd Proxy）组成，核心职责是在不可靠的网络环境中确保服务间通信的可靠性。确保服务间通信的可靠性并不是什么高深的技术，业内追捧服务网格原因在于，实现上述目标的整个过程无需手动配置，对应用完全透明。

接下来，笔者将从 Sidecar 自动注入、流量透明劫持、实现可靠通信三个方面展开讨论数据平面的设计原理。
